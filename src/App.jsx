...
import { fetchData, setCurrentPage, setStatus, setProductPerPage } from './store/slices/apiSlice';
import ProductList from './components/ProductList/ProductList';
import DropDown from './components/DropDown/DropDown';

function App() {
  const {items, currentPage, productsPerPage, status} = useSelector(state => state.getData);
  const dispatch = useDispatch();

  useEffect(() => {
    if(status === 'error' || ( status !== 'loading' && status !== 'resolved')) {
      dispatch(fetchData({action: "get_ids", params: {"offset": currentPage, "limit": productsPerPage}}))
    }
    if(items.length < 50 && status === 'resolved') {
      dispatch(setProductPerPage(productsPerPage + 1));
      dispatch(fetchData({action: "get_ids", params: {"offset": currentPage, "limit": productsPerPage}}));
    } else if(items.length === 50 && status === 'resolved') {
